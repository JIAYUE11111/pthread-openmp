...
{/*
	...
	#endif
	*/
	for (NUM_THREADS = 2; NUM_THREADS <= 12; NUM_THREADS++)
	{
		cout << "THREADS" << NUM_THREADS << endl;
		res_stream << NUM_THREADS;
...
void init_data()
{
	SHUJU = new float *[N], matrix = new float *[N];
	for (int i = 0; i < N; i++)
		SHUJU[i] = new float[N], matrix[i] = new float[N];
	for (int i = 0; i < N; i++)
		for (int j = i; j < N; j++)
			SHUJU[i][j] = rand() * 1.0 / RAND_MAX * L;
	for (int i = 0; i < N - 1; i++)
		for (int j = i + 1; j < N; j++)
			for (int k = 0; k < N; k++)
				SHUJU[j][k] += SHUJU[i][k];
